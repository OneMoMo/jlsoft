    ...
    "FJFPZP" : {
    		...
    		"listener": {
    			"loadFile" : function(data) {
    				...
    				var size=204800-data[data.length-1].FILE_SIZE*1;
    				//var name=data[data.length-1].FILE_DESC;
    				if(size<0){
    					JL.tip("导入照片大小超过200KB","info");
    ...
    "FJGCZP" : {
		...
		"listener": {
			"loadFile" : function(data) {
				...
				var size=3*1024*1024-data[data.length-1].FILE_SIZE*1;
				if(size<0){
//					$("#d_FJGCZP").find(".img_main").find("li")[length-1].remove();   //删除照片
					JL.tip("导入照片大小超过3M","info");
					return true;
				}
			}
		}
