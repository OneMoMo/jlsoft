	...
	"toolbar" : {
		...
		"buttons": {
			...
			"jlTJ":{
				...
				"func":function(){
					debugger;
					if(makeGCDLD.workflow.bz02=="跟进"){
						if((makeGCDLD.getTab().find("input[name='YQCS']:not(:disabled)").val()*1)>0){
							if(JL.isNull(makeGCDLD.getTab().find("textarea[name='GC_GJQK']:not(:disabled)").val())){
								JL.tip("详细描述不能为空","info");
								makeGCDLD.getTab().find("#jlSaveForm").hide();
								return false;
							}
						}
					...
					makeGCDLD.getTab().find("#jlSaveForm").click();
				}
			},
      ...
      //删除
			"jlDeleteForm":{
					"success":function(data,tip){
						...
						makeGCDLD.getTab().find("#jlTJ").hide();
					}
...
makeGCDLD.setAfterInit(function(){
	...
	//撤回时页面不显示提交按钮
	if(!(makeGCDLD.getTab().find("#jlCancel").css("display")=="none")){
		makeGCDLD.getTab().find("#jlTJ").hide();
	}
	//控制审核后的单子不能被删除
	if(makeGCDLD.workflow.bz02=="跟进"){
	...
	//制单不显示删除按钮
	if(makeGCDLD.workflow.bz02=="制单"&&JL.isNull(makeGCDLD.sk01)){
		makeGCDLD.getTab().find("#jlDeleteForm").hide();
	}
	...
	var jxsjbData = makeGCDLD.getSqlResult(queryField, "JXSJB", "CZY", "scm/pub/search");
	if (jxsjbData.data.length > 0) {
		if (jxsjbData.data[0]["JXSJB"] >= 1) {// 是否为经销商 
	...
	for(var i = 0; i < configData.length;i++){
		...
		/*if(configData[i].JLCO08 == "BHQ_QDHT"){
			//0湖南   1上海   2海南
			if(configData[i].JLCO04 == "0"){
				 var BHQ = makeGCDLD.getTab().find("input[name='BHQ']:not(:disabled)").val();
				 if(BHQ<=0){
					 makeGCDLD.find("#QDHT").hide();
				 }
			}
		...
		//湖南个性化需求
		//作废的单据隐藏签订合同按钮和提交按钮
		if(configData[i].JLCO08 == "ZFDJ"){
			if(configData[i].JLCO04 == "0"){
                var ZFZT = makeGCDLD.getTab().find("input[name='ZFZT']").val();
                if("1" == ZFZT){
                    makeGCDLD.find("#QDHT").hide();
                    makeGCDLD.getTab().find("#jlTJ").hide();
                }
	...
	//面积匹数
	var configMJPS = JL.getJLconf({"DJLX":"GCDLD"});
	for(var i = 0; i < configMJPS.length;i++){
		debugger;
	
		if(configMJPS[i].JLCO08 == "MJ"){
			if(configMJPS[i].JLCO04 == "1"){
				makeGCDLD.getTab().find("#mj").show();
			}else{
				makeGCDLD.getTab().find("#mj").hide();
			}
		}

		if(configMJPS[i].JLCO08 == "PS"){
			if(configMJPS[i].JLCO04 == "1"){
				makeGCDLD.getTab().find("#ps").show();
			}else{
				makeGCDLD.getTab().find("#ps").hide();
			}
		}
	}
	
	var configData_YQ = JL.getJLconf({"DJLX":"GCDLD_YQ_JY"});
	 debugger;
	 //延期日期不为空 并且 作废标记为空的时候
	 /*if (!JL.isNull(makeGCDLD.data['DQR']) || JL.isNull(makeGCDLD.data['ZFBJ'])){
		 //GCDLD01
		var ajaxJson = {}; 
		var data={}; 
		data["GCDLD01"] = makeGCDLD.data['GCDLD01'];
		ajaxJson["src"] = pubJson.getURL("FormUrl") + "/GCDLDYQJK/getGCDLDYQ.do";
	 ...
	 // makeGCDLD.getTab().find("div[id='OA_DSH']").show();
	 debugger;
	 ...
	 if (makeGCDLD.getTab().find("input[name='PFR']").is(":not(:disabled)")) {
			...
			makeGCDLD.getTab().find("li[id='YQAN']").hide();//隐藏保护期
			makeGCDLD.getTab().find("div[id='GC_GJ']").hide();
			makeGCDLD.getTab().find("div[id='GC_HF']").hide();
			makeGCDLD.getTab().find("#jlDeleteForm").hide();
		    ...
		    var DYDLBJ=makeGCDLD.getPluginObj("DYDLBJ").getData();
         	var JY_YQTS;var FDYDL_YQ;
			if(JL.isNull(configData_YQ)){
				//如果查不到数据则隐藏延期功能BHQXS
				makeGCDLD.getTab().find("li[name='BHQXS']").hide();
			}else{
				$(configData_YQ).each(function(i,n){
					if(n.JLCO08 == "JY_YQTS"){
						JY_YQTS = n.JLCO04;
					}else if(n.JLCO08 == "FDYDL_YQ"){
                        FDYDL_YQ = n.JLCO04;
					}
				});
			}
         	 var DYDL=0;
			 if(DYDLBJ.key==0 || FDYDL_YQ==0){
                 DYDL = 1;
			 }
			 ...
			 if(DYDL==1){
				 ...
				 //参数控制上海格力登录序号默认为空
				 var configDHXH = JL.getJLconf({"DJLX":"GCDLD_DLXH"});
				 for(var i = 0; i < configDHXH.length;i++){
					debugger;
					if(configDHXH[i].JLCO08 == "DLXH"){
						if(configDHXH[i].JLCO04 == "0"){
							makeGCDLD.getTab().find("input[name=DLXH]").val("");
						}
					}
				 ...
				 if(BHQCZ==""){
					 debugger;
					 makeGCDLD.getTab().find("input[name='BHQ']:not(:disabled)").val(JY_YQTS);
					 makeGCDLD.getTab().find("input[name='DQR']:not(:disabled)").val(JL.formatDate(JY_YQTS,1));
				 }
...
//		 makeGCDLD.getPluginObj("SH").disabled();
		 if(JL.isNull(configData_YQ)){
             ...
             makeGCDLD.getTab().find("li[name='BHQXS']").attr("style","display:none");
             $(configData_YQ).each(function(i,n){
               if(n.JLCO08 == "FDYDL_YQ"){
                     if(n.JLCO04==0){
                         makeGCDLD.getTab().find("li[name='BHQXS']").attr("style","");
                     }
                     return false;
                 }
             });
			...
			if (jxsjbData.data.length > 0) {
				if (jxsjbData.data[0]["JXSJB"] >= 1) {// 是否为经销商
					...
					makeGCDLD.getPluginObj("SH").disabled();
					makeGCDLD.getTab().find("#DLXH span").attr("style","display:none");
					makeGCDLD.getPluginObj("DYDLBJ").disabled();
					makeGCDLD.getTab().find("li[name='YSJXSYC']").attr("style","display:none");
					makeGCDLD.getTab().find("input[name=YJNR]").attr("readonly","readonly");
				}
			}
			//海南需求   非第一登录也有保护期  暂时通过参数控制上海和湖南非第一登录没有保护期
		    // 在上方获取参数中控制，FDYDL_YQ字段配置成0则显示  否则影藏
			// var DYDLBJ=makeGCDLD.getPluginObj("DYDLBJ").getData();
			// if(DYDLBJ.key==1){
			// 	makeGCDLD.getTab().find("li[name='BHQXS']").attr("style","display:none");
			// }
...
//		    makeGCDLD.getTab().find("li[id='YQAN']").hide();
			makeGCDLD.getTab().find("#jlNewForm").hide();
			makeGCDLD.getTab().find("#jlSaveDraft").hide();
			makeGCDLD.getTab().find("#jlDeleteForm").hide();
			var SPLB = makeGCDLD.getPluginObj("SPLB");
...
//通过参数控制家用及商用及延期次数和延期天数
    {
        ...
        "func": function(){
            // if(makeGCDLD.getPluginObj("DYDLBJ").getData().key=="0"){
                var configData = JL.getJLconf({"DJLX":"GCDLD_YQ_JY"});
                //var configData = config.data.linkedCaseInsensitiveMapList;
                var ExtensionDays,DelayTimes,JY_BHQ=5;
                ...
                var BHQ=makeGCDLD.getTab().find("input[name='BHQ']:not(:disabled)").val()*1;
                var queryField = {};
                queryField["GCDLD01"] = makeGCDLD.getTab().find("input[name='GCDLD01']").val();
                queryField["GSXX01"] = userInfo["PCRM_GSXX01"];
                if(BHQ == 0){
                    JL.tip("单据已超期！");
                    return false;
                ...
                if(BHQ<JY_BHQ){
					...
					if (yqcs2>DelayTimes) {
						...
						makeGCDLD.getTab().find("input[name='YQCS']:not(:disabled)").val(yqcs2);
						//延期标记赋值
						// makeGCDLD.getTab().find("input[name='YQBJ']:not(:disabled)").val("1");
						return false;
					}
					JL.confirm("确认延期？", function(){
						makeGCDLD.getTab().find("input[name='DQR']:not(:disabled)").val(JL.formatDate(ExtensionDays + BHQ, 1));
						makeGCDLD.getTab().find("input[name='BHQ']:not(:disabled)").val(ExtensionDays + BHQ);
						makeGCDLD.getTab().find("input[name='YQRQ']").val(JL.formatDate(0, 2));
						queryField["YQCS"] = makeGCDLD.getTab().find("input[name='YQCS']:not(:disabled)").val();
						queryField["DQR"] = makeGCDLD.getTab().find("input[name='DQR']:not(:disabled)").val();
						queryField["BHQ"] = makeGCDLD.getTab().find("input[name='BHQ']:not(:disabled)").val();
						queryField["YQRQ"] = makeGCDLD.getTab().find("input[name='YQRQ']").val();
						var resultData = makeGCDLD.getSqlResult(queryField, "MONGO_GCDLD", "GCDLDBHQYQ", "scm/gcgl/gcdl/search");
						debugger;
						if (resultData["data"]["resultlist"] == "延期成功") {
							JL.tip("延期成功！");
						}
					});
                    ...
                    JL.tip("保护期少于"+JY_BHQ+"天，才能进行延期操作");
                    return false;
                }
            // }else{
            //     JL.tip("不是第一登陆，不存在保护期");
	...
	if(resultData.data.length!=0){
		if(resultData.data[0]["JXSJB"]==2){//二级经销商
			...
			if(resultData01.data.length!=0){
				...
				makeGCDLD.getTab().find("input[name='JXSGCBH']").val(CJBM);
		...
		if(resultData.data[0]["JXSJB"]==1){//一级经销商
			var queryField02={};
			var resultData02 = makeGCDLD.getSqlResult(queryField02, "WLDW", "KH_ALLMX", "scm/pub/search");	
			if(resultData02.data.length!=0){
				var JXSMC = resultData02.data.resultlist[0]["WLDWMC"];
				var JXSDH = resultData02.data.resultlist[0]["JXSDH"];
				var WLDW01 = resultData02.data.resultlist[0]["WLDW01"];
				var WBBM = resultData02.data.resultlist[0]["WBBM"];
				makeGCDLD.getPluginObj("WLDW01").setData(WLDW01);
				makeGCDLD.getTab().find("input[name='JXSMC']:not(:disabled)").val(JXSMC);
				makeGCDLD.getTab().find("input[name='JXSDH']:not(:disabled)").val(JXSDH);
				makeGCDLD.getTab().find("input[name='JXSGCBH']").val(WBBM);
