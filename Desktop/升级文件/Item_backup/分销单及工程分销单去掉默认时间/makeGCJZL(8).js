...
//	 makeGCJZL.getTab().find("input[name='BM01']:not(:disabled)").val(userInfo["PCRM_BM01"]);
	 makeGCJZL.getTab().find("input[name='BM01']:not(:disabled)").val(userInfo["SCM_BM01"]); //BM01取userInfo["SCM_BM01"]
	...
	"toolbar" : {
		...
		"buttons": {
			...
			"jlTJ":{
				...
				"func":function(){
					debugger;
					if(makeGCJZL.getTab().find("input[name='ZLSHBH']").is(":not(:disabled)")) {
						var configData = JL.getJLconf({"DJLX":"makeGCJZL"});
						for(var i=0;i<configData.length;i++){
							if(configData[i].JLCO08 == "ZLSHBH"){
								if(configData[i].JLCO04 == "0"){
									//必填
									if(JL.isNull(makeGCJZL.getTab().find("input[name='ZLSHBH']:not(:disabled)").val())
											&& makeGCJZL.getPluginObj("SH").getData().key=="Y"){
										JL.tip("请填写资料审核编号","info");
										return false;
										makeGCJZL.getTab().find("#jlSaveForm").hide();
									}
								}
...
//						alert(FXSL+"  "+BCZLSL);
						makeGCJZL.getPluginObj("LX").setCell(FXSL-BCZLSL,i,makeGCJZL.getPluginObj("LX").getRowIndexByID("PFSL"));//6
	...
	"FJTMSC" : {
        ...
        "listener" : {
				...
				if(!JL.isNull(resultData)){
					...
					if (resultData.length > 0){
						...
						for(var i=0;i<resultData.length;i++){
							var SNJ_String = resultData[i]["SNJ"].replace(/\s/g,'');;
							var SWJ_String = resultData[i]["SWJ"].replace(/\s/g,'');;
						...
						if(DRTS==resultData.length){
							...
							$(configData).each(function(i,n){
								if(configData[i].JLCO08 == "BCZLSL"){
									if(configData[i].JLCO04 == "0"){
										//禁用资料数量不可编辑
										makeGCJZL.getPluginObj("LX").disabledColumn("BCZLSL",true);
									}
								}
							});
						}else{
			...
			"removefile" : function(data) {
				//禁用资料数量不可编辑
				makeGCJZL.getPluginObj("LX").disabledColumn("BCZLSL",false);
			}
 ...
 "LX" : {
         ...
         "tittles" : "商品列表", 
          "headers": [
               ...
               {"id": "PFSL","name": "批复数量","width": 100,"align":"right","hidden":true},
               ...
               {"id": "FXDJ","name": "分销单价","width": 150,"align":"right"},
               {"id": "FXJE","name": "分销金额","width": 150,"align":"right","summary" : "sum"},
         ...
         "listener": {
        	 "loadRow":function(Plugin, data, index, dl){
        		 ...
        		 if(data["BZJDJ"]*1==0 || JL.isNull(data["BZJDJ"])){
        			 //当保证金单价为0时设置界面显示
        			 if(!makeGCJZL.pid){
        				// makeGCJZL.getPluginObj("LX").setCell(0, index, Plugin.getRowIndexByID("WSJSL"));//14
        				 Plugin.setCell(0, index, Plugin.getRowIndexByID("BZJDJ"));//14
        			 }
        		 }
        		 if(data["BCFHBZJ"]*1==0 || JL.isNull(data["BCFHBZJ"])){
        			 //当保证金单价为0时设置界面显示
        			 if(!makeGCJZL.pid){
        				// makeGCJZL.getPluginObj("LX").setCell(0, index, Plugin.getRowIndexByID("SYBTSL"));//15
        				 Plugin.setCell(0, index, Plugin.getRowIndexByID("BCFHBZJ"));//14
        			 }
        		 }
         	  },
         	  "updateRow":function(){
         		 ...
         		 if(!JL.isNull(makeGCJZL.getPluginObj("FJTMSC").getData())){
        			 Plugin.disabledCell("BCZLSL",index,true);
        		 }
         	  },
		      "rowclick":function(){
...
makeGCJZL.getMrzt=function(startData,endData){
	...
	if(!JL.isNull(startData) && !JL.isNull(endData)){
		...
		var configData = JL.getJLconf({"DJLX":"makeGCJZL"});
		for(var i=0;i<configData.length;i++){
			if(configData[i]["JLCO08"]=="nomalTime"){
				one=configData[i]["JLCO04"]*1;
			}
			if(configData[i]["JLCO08"]=="laterTime"){
				two=configData[i]["JLCO04"]*1;
			}
		}
		var sDate = new Date(startData);
		var eDate = new Date(endData);
