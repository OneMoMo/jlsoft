        ...
        "listener" : {
        	"loadFile":function(data){
				...
				if(!JL.isNull(resultData)){
					debugger;
					var LX = makeGCJZL.getPluginObj("LX").getData();
					//var SCLB = makeGCJZL.getPluginObj("FJTMSC").getData();
					
					resultData = resultData.data.returnList;
					console.info(resultData);
					if(resultData.length>0){
						LX.disabledCell("BCZLSL",resultData.length,true);
					}else{
						LX.disabledCell("BCZLSL",resultData.length,false);
					}
					if (resultData.length > 0){
						...
						for(var i=0;i<resultData.length;i++){
							...
							var SWJ_String = resultData[i]["SWJ"];
