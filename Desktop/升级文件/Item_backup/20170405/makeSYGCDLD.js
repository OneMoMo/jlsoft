				 ...
				 //默认第一登陆，登陆序号为1
					var DYDL=1;
		  			...
		  			if(BHQCZ==""){
		  				...
		  				makeSYGCDLD.getTab().find("input[name='BHQ']:not(:disabled)").val(ExtensionDays);
		  				makeSYGCDLD.getTab().find("input[name='DQR']:not(:disabled)").val(JL.formatDate(ExtensionDays,1));
		  			}
			 }
	 }
			...
			if(BHQ<=SY_BHQ){
				...
				if(yqbj==0){
					...
					if (yqcs2>DelayTimes) {
						...
						makeSYGCDLD.getTab().find("input[name='YQCS']:not(:disabled)").val(DelayTimes);
						//延期标记赋值
						makeSYGCDLD.getTab().find("input[name='YQBJ']:not(:disabled)").val("1");
						return false;
					}
					makeSYGCDLD.getTab().find("input[name='DQR']:not(:disabled)").val(JL.formatDate(ExtensionDays+BHQ,1));
					makeSYGCDLD.getTab().find("input[name='BHQ']:not(:disabled)").val(ExtensionDays+BHQ);
				...
				JL.tip("保护期少于"+SY_BHQ+"天，才能进行延期操作");
				return false;
			}
		}else{
			JL.tip("不是第一登陆，不存在保护期");
