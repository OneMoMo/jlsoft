var data_name = $(".pagr_content[data-name^='chaoSong']:not(:hidden)").attr("data-name");
eval("var "+data_name+" = JL.JLForm();");
var chaoSong = eval(data_name);

chaoSong.BUTTONS = {};
chaoSong.BUTTONSIZE = {};
chaoSong.XWLIST = {};

chaoSong.setButton = function(){
...
chaoSong.setPlugin({
	...
	"LIST": {
		...
		"listener": {
			...
			"rowclick": function(thisPlugin, chaoSongRow, rowIndex){
				if($(window.event.target).closest("[groupid='CZ']").length == 0){
					chaoSong.openForm(chaoSongRow);
				}
...
chaoSong.openForm = function(chaoSongRow){
	chaoSongRow.CD01 = chaoSongRow.GZL01;
	chaoSongRow.CD02 = chaoSongRow.GZL02 + "("+ chaoSongRow.TBLNAME +"-"+ chaoSongRow.JLBH+")";
	chaoSongRow.YWSJ = {"bdbh":chaoSongRow.TBLNAME,"jlbh":chaoSongRow.JLBH,"PCRM_CH":false};
	chaoSongRow.CS = true;
	loadPage(chaoSongRow,{"CD02":chaoSongRow.GZL02+"抄送"});
}
...
chaoSong.selectDBSY = function(queryField){
	var form = this;
	var XmlData = {};
	...
	var ajaxJson = {};
	ajaxJson["src"] = "jlquery/selectDBSY_CC.do";
		        	            	 ...
		        	            	 "listener":{
		        	            		 "click": function(thisPlugin, rowIndex, obj, itemIndex){
		        	            			 if(thisPlugin.getData(rowIndex, "BZ03") != "1"){
		        	            				 ...
		        	            				 chaoSong.openForm(thisPlugin.getData(rowIndex));
		        	            			 }
		        	            		 }
		        	            	 }
		        	             } 
