...
Kalendae.prototype = {
	...
	setDisabledDay : function (days) {
		this.settings.disabledDays = days; 
		this.draw();
		return false;
	},
	...
	draw : function draw() {
		...
		do {
			...
			do {
				...
				var GFEDCBA = day;
				GFEDCBA = GFEDCBA.format(this.settings.format || 'YYYY-MM-DD');
				if (!( $.inArray(GFEDCBA, this.settings.disabledDays) != -1 ||  this.blackout(day) || this.direction(day) || (day.month() != month.month() && opts.dayOutOfMonthClickable === false)) || s>0){
					klass.push(classes.dayActive);
					
					if (s && opts.mode == 'range' && $.inArray(GFEDCBA, this.ABCDEFG) == -1) {
						this.ABCDEFG.push(GFEDCBA);
					}
				}
